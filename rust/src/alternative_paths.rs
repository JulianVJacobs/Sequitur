...
pub fn detect_swap_squares(
	...
	for i in 0..n {
		for j in (i + 1)..n {
			...
			let val_ii = lookup.get(&(i, i)).copied().unwrap_or(0);
			let val_jj = lookup.get(&(j, j)).copied().unwrap_or(0);
			let val_ij = lookup.get(&(i, j)).copied().unwrap_or(0);
			let val_ji = lookup.get(&(j, i)).copied().unwrap_or(0);
			
			if val_ii > 0 && val_jj > 0 && val_ij > 0 && val_ji > 0 {
				let delta = (val_ij + val_ji) as f64 - (val_ii + val_jj) as f64;
...
pub fn build_swap_graph(squares: &[SwapSquare]) -> HashMap<usize, HashSet<usize>> {
	...
	for square in squares {
		graph.entry(square.i).or_insert_with(HashSet::new).insert(square.j);
		graph.entry(square.j).or_insert_with(HashSet::new).insert(square.i);
	}
...
pub fn is_cycle(component: &[usize], graph: &HashMap<usize, HashSet<usize>>) -> bool {
	...
	fn has_cycle_dfs(
		...
		if let Some(neighbors) = graph.get(&node) {
			for &neighbor in neighbors {
				...
				if !visited.contains(&neighbor) {
					if has_cycle_dfs(neighbor, Some(node), visited, rec_stack, graph, component_set) {
						return true;
					}
	...
	has_cycle_dfs(component[0], None, &mut visited, &mut rec_stack, graph, &component_set)
}
...
mod tests {
	...
	fn detects_cycle_in_component() {
		...
		let squares = vec![
			SwapSquare { i: 0, j: 1, delta: 0.0 },
			SwapSquare { i: 1, j: 2, delta: 0.0 },
			SwapSquare { i: 2, j: 0, delta: 0.0 },
		];

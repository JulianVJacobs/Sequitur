    ...
    if quality_map is None:
        ...
        for c,r in zip(cols,rows):
            if b: 
                seq += reads_map[c][:-O[(c,r)]]
                b = False
            prefix,pre_q = reads_map[c][-O[(c,r)]:],quality_map[c][-O[(c,r)]:]
            suffix,suf_q = reads_map[r][:-O[(c,r)]],quality_map[r][:-O[(c,r)]]
            for base in range(O[(c,r)]):
                if pre_q[base] >= suf_q[base]: seq += prefix[base]
                else: seq += suffix[base]
        seq += reads_map[rows[-1]][-O[(cols[-1],rows[-1])]:]
    ...
    if do_time: 
        ...
        SeqIO.write([
            SeqRecord(
                ...
                id="sequitur",
                description=""
                )], f"/workspace/sequitur/data/output/Raphanus sativus_NC_018551.1/{file}.sequitur.out.fasta", "fasta")
        return seq, t
    return seq
...
if __name__ == "__main__":
    ...
    def get_reads():
        return chain(
            (read.seq.upper() for read in SeqIO.parse(f"/workspace/sequitur/data/input/Raphanus sativus_NC_018551.1/{args.input}.1.fastq",'fastq')),
            (read.seq.upper().reverse_complement() for read in SeqIO.parse(f"/workspace/sequitur/data/input/Raphanus sativus_NC_018551.1/{args.input}.2.fastq",'fastq'))
            )
    def get_ids():
        return chain(
            (read.id for read in SeqIO.parse(f"/workspace/sequitur/data/input/Raphanus sativus_NC_018551.1/{args.input}.1.fastq",'fastq')),
            (read.id for read in SeqIO.parse(f"/workspace/sequitur/data/input/Raphanus sativus_NC_018551.1/{args.input}.2.fastq",'fastq'))
            )
    def get_qualities():
        return chain(
            (read.letter_annotations["phred_quality"] for read in SeqIO.parse(f"/workspace/sequitur/data/input/Raphanus sativus_NC_018551.1/{args.input}.1.fastq",'fastq')),
            (read.letter_annotations["phred_quality"] for read in SeqIO.parse(f"/workspace/sequitur/data/input/Raphanus sativus_NC_018551.1/{args.input}.2.fastq",'fastq'))
            )
    if not os.path.exists(f"/workspace/sequitur/data/output/{args.input}.sequitur.csv") or os.path.getsize(f"/workspace/sequitur/data/output/{args.input}.sequitur.csv") == 0:
        with open(f"/workspace/sequitur/data/output/{args.input}.sequitur.csv",'a') as f:
    ...
    with open(f"/workspace/sequitur/data/output/{args.input}.sequitur.csv",'a') as f:
        for _ in range(100):
            suf_arr,suf_arr_ind,t1 = build_suffix_array(get_reads(),do_time=True)
            B, O,t2 = create_bipartite_adjacency_matrix(get_reads(),zip(get_reads(),(index for index,_ in enumerate(get_reads()))),suf_arr=suf_arr,suf_arr_ind=suf_arr_ind,do_time=True)
            start = time.time()
            B = coo_matrix((list(B.values()),list(zip(*B.keys())))).T
            if B.shape[0] < len(list(index for index, _ in enumerate(get_ids()))): B = vstack([B,coo_matrix((1, B.shape[1]),dtype=B.dtype)])
            if B.shape[1] < len(list(index for index, _ in enumerate(get_ids()))): B = hstack([B,coo_matrix((B.shape[0], 1),dtype=B.dtype)])
            t2 += time.time() - start
            seq_,t3 = find_lower_diagonal_path(B,O,zip(get_ids(),get_reads()),get_ids(),get_ids(),file=args.input,do_time=True,quality_map=zip(get_ids(),get_reads()))
